...
class RfMockupServer(BaseHTTPRequestHandler):
    ...
    def construct_path(self, path, filename):
        ...
        print("server-shortform:86: ",self.server.shortForm)
        rpath = clean_path(path, self.server.shortForm)
        print("rpath:88: ",rpath)
    ...
    def do_GET(self):
        ...
        if self.path == "/" and self.server.shortForm:
            print("The changed line",self.server.shortForm)
            self.send_response(404)
            self.end_headers()
...
def main():
    ...
    print("line number 859 " , mockDirPath)
    ...
    if not shortForm:
        ...
        if os.path.isdir(slashRedfishDir) is not True:
            print("line number 859 " , mockDirPath," ",slashRedfishDir)
